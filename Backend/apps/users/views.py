...
class RoleDestroyUpdateView(generics.RetrieveUpdateDestroyAPIView):
    queryset = Role.objects.all()
    serializer_class = RoleSerializer
    permission_classes = [AllowAny]
        ...
        try:
            ...
            if serializer.is_valid():
                with transaction.atomic():
                    ...
                    activation_link = (
                        f"http://127.0.0.1:8000/users/activate/{uid}/{token}/"
                    )
                    ...
                    send_email_notification(
                        request, user, email_subject, email_template, activation_link
                    )
